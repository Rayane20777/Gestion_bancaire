<?php
        ...
        require("../app/controllers/Agency.php");
...
<div class="bg-gray-100">
    ...
    <form action="../app/controllers/Agency.php" method="post">
        <input type="text" name="longitude" id="longitude" placeholder="longitude">
        <input type="hidden" name="action" value="addAgency">
        <input type="text" name="latitude" id="latitude" placeholder="latitude">
        <input type="text" name="bank_id" id="bank_id" placeholder="bank_id">
        <input type="text" name="address_id" id="address_id" placeholder="address_id">
        ...
        <tbody>
            <?php foreach($agencys as $agency): ?>
                <tr>
                    ...
                    <td><?=$agency['address_id']?></td>
                    <form action="" method="post">
                        <input type="hidden" name="action" value="editAgency">
                        <input type="hidden" name="id" value="<?=$agency['id']?>">
                        <td>
                            <input type="text" name="longitude" value="<?=$agency['longitude']?>">
                        </td>
                        <td>
                            <input type="text" name="latitude" value="<?=$agency['latitude']?>">
                        </td>
                        <td>
                            <input type="text" name="bank_id" value="<?=$agency['bank_id']?>">
                        </td>
                        <td>
                            <input type="text" name="address_id" value="<?=$agency['address_id']?>">
                        </td>
                        <td>
                        <button type="submit" name="edit" onclick="return confirm('Are you sure you want to edit this bank?')">Edit</button>
                        </form>
                    <td><form action="../app/controllers/Agency.php" method="post">
                            <input type="hidden" name="delete_id" value="<?= $agency['id'] ?>">
                            <input type="hidden" name="action" value="deleteAgency">
                            <button type="submit" name="delete" onclick="return confirm('Are you sure you want to delete this agency?')">Delete</button>
                        </form></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
